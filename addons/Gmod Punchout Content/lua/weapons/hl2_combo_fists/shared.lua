...
SWEP.Author			= "Nortonv8"
SWEP.Instructions	= "LMB = Left hander, RMB = Right hander."
SWEP.Contact		= "nortonv8@gmail.com"
SWEP.Purpose		= "To beat the crap outta people!"
...
function SWEP:Think()
	if self:GetNWInt("Right") >= 400 then -- If we hit an NPC/Player with our left hand 2 times, then
		self.ComboActivated = true -- Let us do a Combo-Hit!
	elseif self:GetNWInt("Left") >= 400 then --If we hit an NPC/Player with our left hand 2 times, then
		self.ComboActivated = true -- Let us do a Combo-Hit!
	end
	
	if (CurTime() < self.QuickHittingTime) then -- If we can keep up the left-right hand swinging/hitting pace, then
		self.Delay 	= 0.5 -- Let us swing abit faster!
		self.Delay2	= 0.5 -- Let us swing abit faster!
	else -- Otherwise
...
function SWEP:PrimaryAttack()
	local extradamage = 2 * (self:GetNWInt("Left") + self:GetNWInt("Right"))
	local trace = self.Owner:GetEyeTrace()
	local phys = trace.Entity:GetPhysicsObject()
	if trace.HitPos:Distance(self.Owner:GetShootPos()) <= 40 then -- If we're in range
		if self.ComboActivated == true then -- If we activate a Combo
			...
			if SERVER then
				trace.Entity:TakeDamage(math.random(2, 5) + extradamage, self.Owner, self.Owner) -- Do more damage
			end
			...
			if SERVER then
				trace.Entity:TakeDamage(math.random(2, 5), self.Owner, self.Owner) -- Do less damage
			end
	...
	self.QuickHittingTime = (CurTime() + 0.5)
...
function SWEP:SecondaryAttack()
	...
	if trace.HitPos:Distance(self.Owner:GetShootPos()) <= 70 then -- If we're in range
		if self.ComboActivated == true then -- If we activate a Combo
			...
			if SERVER then
				trace.Entity:TakeDamage(math.random(5, 9) + extradamage, self.Owner, self.Owner) -- Do more damage
			end
			self.Weapon:SendWeaponAnim( ACT_VM_SECONDARYATTACK ) -- Display a different animation
			...
			if SERVER then
				trace.Entity:TakeDamage(math.random(5, 9), self.Owner, self.Owner) -- Do less damage
			end
			self.Owner:ViewPunch(Angle(-1.5, 2.0, 0))
	...
	self.QuickHittingTime = (CurTime() + 0.6)
	self.Owner:SetAnimation( PLAYER_ATTACK1 )
end 